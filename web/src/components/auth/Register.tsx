...
export default function Register() {
  ...
  const parseErrorMessage = (err: unknown): string => {
    if (!(err instanceof Error)) return t("auth.errors.registrationFailed", "Registration failed");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.password.length < 4) {
      setError(t("auth.errors.passwordTooShort", "Password must be at least 4 characters"));
      return;
    }
      ...
      >
        <Card className="w-full bg-white/95 dark:bg-gray-850/95 border-gray-200 dark:border-gray-800 shadow-xl">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-gray-600 dark:text-gray-400">{t("auth.createYourAccount", "Create your account")}</CardDescription>
          </CardHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="username" className="text-gray-700 dark:text-gray-300">{t("auth.username", "Username")}</Label>
                  <Input
                    id="username"
                    type="text"
                ...
                <div className="space-y-2">
                  <Label htmlFor="password" className="text-gray-700 dark:text-gray-300">{t("auth.password", "Password")}</Label>
                  <Input
                    id="password"
                    type="password"
